...
export function WikiSpaces(): ReactElement {
	return (
		<>
			...
			<p>
				Spaces in Pandora can be public or private, which affects who can see them. There are two different public space visibility settings: "Public only while an
				admin is inside and online" and "Public only while someone is inside and online". If the public condition is no longer fulfilled, the space is temporarily no longer
				publicly listed in the list of spaces. Despite that, everyone inside can still directly invite other users from their contacts list to the unlisted public
				space (see <Link to='#SP_Space_invites'>"Space invites"</Link> section).<br />
				As spaces often no longer serve the intended and described purpose while the owner/admins went offline, the public-only-with-admin setting helps to reduce the amount
				of "dead" spaces in the list of spaces.
			</p>
			<ul>
				<li>The default when creating a new space is private.</li>
				<li>Private spaces are only showing up in the list of spaces for owners and admins of that space as well as users whose account is on the "allowed users" list of the space.</li>
			...
			<p>
				Spaces in Pandora that are public and visible/listed can be joined unless a user is banned from it or the space is full.
			...
			<ul>
				...
				<li>You can set the space visibility. The default is that the space is private.</li>
